...
def get_or_create_event_registration(
    ...
    If created, the registration status is set to 'CONFIRMED' or 'PENDING' based on the 'with_confirmed' flag.
    ...
    if created:
        send_event_registration_email.delay(user.id, event.id, status)
